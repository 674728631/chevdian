...
public class WxUtil {
   ...
   /*     Map<String, Object> resultMap = new TreeMap<String, Object>();
     ...
     * @return
     */
    public JSONObject coverH5Pay(Object prepayId) {
        SortedMap<String, String> treeMap = new TreeMap<>();
        String currTime = DateUtil.curTime(new Date());
        treeMap.put("appId", weixinProperties.getAppid());
        treeMap.put("timeStamp", currTime);
        treeMap.put("nonceStr", currTime.substring(8, currTime.length()) + RandomUtil.createNumberCode(4) + "");
        ...
        return (JSONObject) JSON.toJSON(treeMap);
    }
     ...
     * @return
     */
    public String reUsePrepay(String orderNo, String userBankCard, String userName, String bankCode, int amt, String desc) throws Exception {
        ...
        es.put("mch_id", weixinProperties.getPartner());//商户号
     ...
     * @return
     */
    public String createSHA1Sign(SortedMap<String, String> esMd5) {
        //排序
        SortedMap<String, String> sortMap = new TreeMap<>(new MapKeyComparator());
        ...
        String params = sb.toString() + "key=" + weixinProperties.getPartnerKey();
     ...
     * @Title: 微信预支付
     */
    public String sendPrepay(long time_start, String basePath, String userIp, String describe, String openId, String orderId, int taotal, String SuJiShu, String deviceNo) throws Exception {
        String prepayid = "";
        SortedMap<String, String> es = new TreeMap<>();
        es.put("appid", weixinProperties.getAppid());
        es.put("mch_id", weixinProperties.getPartner());  //商户号
        ...
        es.put("time_start", time_start + ""); // 订单生成时间
     ...
     * @throws Exception
     */
    public String httpClientPostSSL(String url, String data) throws Exception {
        ...
        keyStore.load(is, weixinProperties.getPartner().toCharArray());
        is.close();
        SSLContext sslcontext = SSLContexts.custom().loadKeyMaterial(keyStore, weixinProperties.getPartner().toCharArray()).build();
        SSLConnectionSocketFactory sslsf = new SSLConnectionSocketFactory(sslcontext, new String[]{"TLSv1"}, null, SSLConnectionSocketFactory.BROWSER_COMPATIBLE_HOSTNAME_VERIFIER);
        CloseableHttpClient httpclient = HttpClients.custom().setSSLSocketFactory(sslsf).build();
...
//        result = HttpPost("", xmlStr);
    ...
    private String reEncTrueName() throws Exception {
        SortedMap<String, String> es = new TreeMap<String, String>();
        es.put("mch_id", weixinProperties.getPartner());//商户号 ,Constant.toReadPro("partner")
        es.put("nonce_str", SuJiShu());                         //随机字符串
     ...
     * @version 1.0
     */
    public String transferPay(String orderNo, String SuJiShu, String openid, String amount, String desc, String spbill_create_ip) throws Exception {
        String transferPayXml = "";
        if (orderNo.length() > 32)
            orderNo = orderNo.substring(orderNo.length() - 31, orderNo.length());
        SortedMap<String, String> es = new TreeMap<>();
        es.put("mch_appid", weixinProperties.getAppid());
        es.put("mchid", weixinProperties.getAppsecret());
        es.put("nonce_str", SuJiShu);
        es.put("partner_trade_no", orderNo);
        es.put("openid", openid);
        es.put("check_name", "NO_CHECK");
        es.put("amount", amount);
        es.put("desc", desc);
        es.put("spbill_create_ip", spbill_create_ip);
        es.put("sign", createSHA1Sign(es));
        String xmlStr = getRequestXml(es);
        transferPayXml = httpClientPostSSL(TRANSFERS_PAY, xmlStr);
//    	transferPayXml = HttpPost(TRANSFERS_PAY, xmlStr);
        return transferPayXml;
    }
